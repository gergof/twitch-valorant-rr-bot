...
class LiveMonitor {
	...
	constructor(config: Config, app: App) {
		...
		this.authProvider.onRefresh(async (userId, token) => {
			...
			if (!credentials) {
				logger.warn(
					'Received refreshed token for unknown Twitch user',
					{
						userId
					}
				);
	...
	private async reconcileEligibleChannels(
		sendWelcome: boolean
	): Promise<void> {
		const em = this.app.orm.em.fork();
		const channels = await em.find(Channel, {
	...
	private async ensureChannelCredentialsAdded(
		channel: Channel
	): Promise<boolean> {
		const em = this.app.orm.em.fork();
		...
		if (!channelWithCredential?.credential) {
			logger.warn(
				'Skipping channel registration because credential is missing',
				{
					channelId: channel.id,
					twitchId: channel.twitchId
				}
		...
		this.authProvider.addUser(channelWithCredential.twitchId, {
			...
			obtainmentTimestamp:
				channelWithCredential.credential.obtainmentTimestamp.getTime(),
	...
	private async addChannel(channel: Channel): Promise<void> {
		...
		this.subscriptions.set(channel.id, {
			channel,
			online: this.listener.onStreamOnline(
				channel.twitchId,
				async event => {
					...
					await this.safelyHandleChannelEvent(
						channel,
						'online',
						async () => {
			...
			offline: this.listener.onStreamOffline(
				channel.twitchId,
				async () => {
					await this.safelyHandleChannelEvent(
						channel,
			...
			chat: this.listener.onChannelChatMessage(
				channel.twitchId,
				this.botId as string,
				async event => {
	...
	private async upsertLiveStream(
		channel: Channel,
		liveStream: HelixStream
	): Promise<Stream> {
		const em = this.app.orm.em.fork();
	...
	private async handleOnline(
		...
		if (
			liveStream.type != 'live' ||
			liveStream.gameId != TWITCH_VALORANT_GAME_ID
		) {
	...
	private async reconcileChannel(
		...
		const liveStream =
			await this.app.twitchApi.streams.getStreamByUserIdBatched(
				channel.twitchId
			);
